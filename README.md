...
- **全平台多画质支持与风控防护 (v1.0.2)**：
  - **哔哩哔哩 (Bilibili)**：短链 `b23.tv` / `bili2233.cn` 递归解析；自动注入 SPI BUVID 防控 Cookie 解决 HTTP 412 拦截；支持 WBI DASH 4K、1080P60 及无 WBI 标准流三级容灾回退；**支持移动端内置安全网页登录（单机免扫码）**与二维码保存相册+直跳B站双模式。
  - **抖音 (Douyin)**：智能遍历 `bit_rate` 列表提取最高码率 **1080P 超清** 原画流，解决预览流仅 700KB 问题；支持 TTWID 自动注册与无水印 MP4 直存。
  - **快手 (Kuaishou)**：支持精选视频、无水印原画直链秒级提取。
- **极致端侧体验与 UI 重构**：
  - 全线程安全架构，彻底解决 JavaBridge 线程调用 WebView 方法与 Toast 异常。
  - 剔除刻板 AI 模板感，采用现代钛黑质感（Obsidian Titanium）、细腻毛玻璃卡片、微弹簧动画（Spring Curves）与流体 Dynamic Island 灵动提示条。
- **原生音视频混流 (Remux)**：
